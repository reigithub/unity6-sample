    ...
    public class MessageBrokerTestView : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            _messageBroker.GetSubscriber<int, MessageBrokerTestValue>()
                .Subscribe(key: 0, handler: msg => { Debug.LogWarning($"Local Message 1: {msg.Value}"); }, new MessageHandlerFilterTestValue())
                .AddTo(this);

            _messageBroker.AddMessageBroker<int, string>();
            _messageBroker.Build();
            _messageBroker.GetSubscriber<int, string>()
                .Subscribe(key: 0, handler: msg => { Debug.LogWarning($"Local Message 2: {msg}"); }, new MessageHandlerFilterTestString())
                .AddTo(this);
            ...
            //     .AddTo(this);
            //
            // Memo: GlobalMessagePipeをそのまま使用した場合は、↑の登録をかき消す事になる...
            // GetPublisher/GetSubscriberするたびに自分のIServiceProviderインスタンスをGlobalMessagePipeに再セットする事で無理やり共存可能
            // _globalMessageBroker.AddMessageBroker<int, string>();
            // _globalMessageBroker.Build();
            // _globalMessageBroker.GetSubscriber<int, string>()
            ...
            _button.onClick.AddListener(() =>
            {
                ...
                _messageBroker.GetPublisher<int, string>().Publish(0, _count.ToString());

                // _globalMessageBroker.GetPublisher<int, MessageBrokerTestValue>().Publish(0, new MessageBrokerTestValue(_count));
                // _globalMessageBroker.GetPublisher<int, string>().Publish(0, _count.ToString());
            });
        }
    }
